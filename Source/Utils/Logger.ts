...
class Logger {
	...
	static Log(str: string, colour: Color = Color("white"), bold = false): void {
		...
		const moduleName = ToConsole("qHancements", Color("cyan"), true);
		const pipe = ToConsole(" | ", Color("gray"), false);
		const text = ToConsole(str, colour, bold);
		console.log(time.str + moduleName.str + pipe.str + text.str, ...time.params.concat(moduleName.params, text.params));
	}
